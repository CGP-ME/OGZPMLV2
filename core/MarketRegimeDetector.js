...
class MarketRegimeDetector extends EventEmitter {
  ...
  calculateVolatility(candles) {
    ...
    const avgPrice = candles.reduce((sum, candle) => sum + c(candle), 0) / candles.length;
  ...
  calculateATR(candles, period = 14) {
    ...
    for (let i = 1; i <= period; i++) {
      const high = h(candles[i]);
      const low = l(candles[i]);
  ...
  calculateTrend(candles, indicators) {
    ...
    const ma20 = this.calculateSMA(candles.map(candle => c(candle)), 20);
    const ma50 = this.calculateSMA(candles.map(candle => c(candle)), 50);
  ...
  calculateSwingTrend(candles, lookback = 10) {
    ...
    for (let i = 2; i < recentCandles.length - 2; i++) {
      ...
      // Swing high
      if (h(candle) > h(recentCandles[i - 1]) &&
          h(candle) > h(recentCandles[i - 2]) &&
          h(candle) > h(recentCandles[i + 1]) &&
          h(candle) > h(recentCandles[i + 2])) {
        highs.push({ index: i, price: h(candle) });
      }

      // Swing low
      if (l(candle) < l(recentCandles[i - 1]) &&
          l(candle) < l(recentCandles[i - 2]) &&
          l(candle) < l(recentCandles[i + 1]) &&
          l(candle) < l(recentCandles[i + 2])) {
        lows.push({ index: i, price: l(candle) });
      }
    }
  ...
  calculateMomentum(candles) {
    ...
    const currentPrice = c(candles[candles.length - 1]);
    const pastPrice = c(candles[candles.length - lookback - 1]);
  ...
  calculatePricePosition(candles) {
    ...
    const highest = Math.max(...recentCandles.map(candle => h(candle)));
    const lowest = Math.min(...recentCandles.map(candle => l(candle)));
    const current = c(candles[candles.length - 1]);
  ...
  calculateADX(candles, period = 14) {
    ...
    for (let i = period; i < candles.length; i++) {
      const prevAvg = this.calculateSMA(candles.slice(i - period, i).map(candle => c(candle)), period);
      const currAvg = this.calculateSMA(candles.slice(i - period + 1, i + 1).map(candle => c(candle)), period);
