...
class IndicatorEngine {
  ...
  getSnapshot() {
    ...
    const dto = {
      ...
      indicators: {
        ...
        obv: s.obv,
        mfi: s.mfi,
        superTrend: s.superTrend?.value,
        superTrendDirection: s.superTrend?.trend,
        price: price,
      },
      candle: lastCandle ? {
        open: _o(lastCandle),
        ...
        timestamp: lastCandle.t ?? lastCandle.timestamp ?? Date.now(),
      } : undefined,
      overlays: {},
    };
  ...
  _updateMFI() {
    ...
    const tp = (_h(c) + _l(c) + _c(c)) / 3;
    const prevTp = (_h(prev) + _l(prev) + _c(prev)) / 3;
    const rawFlow = tp * (_v(c) || 0);
  ...
  _updateVWAP() {
    ...
    const typical = (_h(c) + _l(c) + _c(c)) / 3;
    const v = _v(c) || 0;
  ...
  _updatePivots() {
    ...
    if (isHigh) this._pushPivot(this.pivots.highs, { i: pivotIndex, t: _t(pivotC), price: _h(pivotC) });
    if (isLow)  this._pushPivot(this.pivots.lows,  { i: pivotIndex, t: _t(pivotC), price: _l(pivotC) });
  ...
  _updateFib() {
    ...
    if (lastHigh.i > lastLow.i) {
      ...
      swing = {
        from: { t: _t(lastHigh), price: lastHigh.price },
        to: { t: _t(to), price: to.price },
        dir: 'down',
      };
    } else {
      ...
      swing = {
        from: { t: _t(lastLow), price: lastLow.price },
        to: { t: _t(to), price: to.price },
        dir: 'up',
      };
    }
  ...
  _fitTrendline(pivots) {
    ...
    return {
      ...
      intercept,
      r2,
      points: pts.map(p => ({ t: _t(p), y: p.price })),
  ...
  _updateSeries(c) {
    ...
    const tFirst = _t(this.candles[firstIdx]) ?? tNow;
