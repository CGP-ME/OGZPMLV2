...
const { c, h, l } = require('./CandleHelper');
...
class FibonacciDetector {
  ...
  findSwings(candles, lookback) {
    ...
    for (let i = candles.length - lookback; i < candles.length; i++) {
      ...
      if (h(candle) > swingHigh) {
        // Validate swing high meets minimum change threshold
        if (swingHigh !== -Infinity && h(candle) - swingHigh >= minSwingChange) {
      ...
      if (l(candle) < swingLow) {
        // Validate swing low meets minimum change threshold
        if (swingLow !== Infinity && swingLow - l(candle) >= minSwingChange) {
          potentialSwings.push({
            type: 'low',
            price: l(candle),
            index: i,
