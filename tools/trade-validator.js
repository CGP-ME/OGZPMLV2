...
const STRATEGY_FILTER = process.env.ISOLATE
...
function validateTrade(trade, candles) {
  ...
  const rawPnl = ((trade.exitPrice - trade.entryPrice) / trade.entryPrice) * 100 - FEES_PCT;
  checks.push({
    name: 'P&L calculation correct',
    passed: Math.abs(trade.pnlPct - rawPnl) < 0.01,
    expected: 'matches raw calc',
    actual: `trade=${trade.pnlPct.toFixed(4)}% raw=${rawPnl.toFixed(4)}%`,
  });
