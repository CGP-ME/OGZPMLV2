...
Scope: Claude bridge session-scoped ledger attribution after closing prior concrete findings:
...
3. Blocked bridge Read/Edit policy access to `.claude/session-state/`.
4. Blocked Bash reads of protected session-state paths through `pre-bash`.
5. Required hook session identity before Warden allows git mutations.
...
- `trai_brain/claude-bridge/policy.js:77-113` blocks protected state before the broad `.claude/` allowlist in `checkPath`.
...
- `trai_brain/claude-bridge/pre-bash.js:301-315` requires session identity before Warden git mutation and passes explicit edit scope.
- `trai_brain/claude-bridge/pre-bash.js:357-374` blocks any disallowed policy path through Bash reads, including protected session-state.
...
- Internal bridge modules are allowed to read/write their own `.claude/session-state/*.json` files. That is hook-owned storage, not a Claude tool-access path.
- A finding about internal ledger modules accessing their own state is not a bypass unless you show how an agent reaches that access through Read/Edit/Bash policy or uses it to cross session attribution.

Attack requirements:
1. Find a concrete path where Session B can still satisfy forced-read or edit attribution using Session A ledger data without already knowing Session A's id from outside the bridge.
...
5. Check sibling state files `read-ledger.json`, `edit-ledger.json`, `task-contract.json`, and `hot-path-proof.json`; identify same bug class only with an exploit path in this threat model.
