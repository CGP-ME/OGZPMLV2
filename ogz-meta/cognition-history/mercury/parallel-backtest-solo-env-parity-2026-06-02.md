Attack this specific backtest-runner parity patch. Do not confirm it. Break it.

Context:
The project rule is that backtest and live execution must share the same execution path. This patch only touches the backtest runner wrapper, not strategy logic. The intended fix is to make tools/parallel-backtest.js target the same registered/exploratory strategies and dormant strategy enable flags as tools/matrix-sweep.js when spawning child run-empire-v2.js workers.

Files/ranges to inspect:
- tools/parallel-backtest.js lines 49-63: results dir/state cleanup moved behind functions.
- tools/parallel-backtest.js lines 69-103: STRATEGIES, parseSoloStrategies, buildDormantStrategyEnableEnv.
- tools/parallel-backtest.js lines 235-252: strategy-sweep roster.
- tools/parallel-backtest.js lines 328-370: selectedSoloStrategy, dormantStrategyEnv, and child worker env construction.
- tools/parallel-backtest.js lines 771-787: require.main guard and exports.
- test/parallel-backtest-solo-env.test.js lines 1-45: focused tests.
...
- core/StrategyOrchestrator.js lines 735-748 then applies TradingConfig pipeline toggles.
