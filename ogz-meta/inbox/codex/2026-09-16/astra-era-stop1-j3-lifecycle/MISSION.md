...
4. Lock-monitor failure exited from `SingletonLock` instead of reporting to the runner.
...
- Singleton integrity loss reports to the runner; it does not terminate independently.
- Backtest success/failure requests the same runner shutdown with exit code `0`/`1`.
- Shutdown stops producers and services, persists state/pattern/journal data, releases the singleton last, and only then exits.
