...
(async()=>{
  ...
  const inherited=cp.execFileSync('git',['diff','--binary','--','modules/DonchianBreakout.js','core/StrategyOrchestrator.js','foundation/ConfigLoader.js'],{cwd:root,maxBuffer:8e6});
  ...
  const record={at:new Date().toISOString(),boundary:'Existing Mercury dependency/Serena AST tools; static candidates, not execution completeness',
    ...
    references:scan.propertyRefs.filter(r=>['entryPeriod','atrPeriod','atrStopMult','allowShorts','trailChannelBars','trailingStopPercent','trailingActivation'].includes(r.property)),
    callers:scan.methodCalls.filter(r=>['evaluate','saveSettings','createExitContract','_checkChannelTrail'].includes(r.method)),
    blast:await Promise.all(['modules/DonchianBreakout.js','core/StrategyOrchestrator.js','foundation/ConfigLoader.js'].map(p=>getBlastRadius(p)))};
