...
(async()=>{
  ...
  const inherited=cp.execFileSync('git',['diff','--binary','--','modules/EMATrendRetest.js','core/StrategyOrchestrator.js','foundation/ConfigLoader.js'],{cwd:root,maxBuffer:8e6});
  ...
  const record={at:new Date().toISOString(),boundary:'Existing Mercury dependency/Serena AST tools; static candidates, not execution completeness',
    ...
    references:scan.propertyRefs.filter(r=>['atrPeriod','slopeLookbackBars','minSlopePct','retestLookbackBars','touchZoneAtr','closeAwayAtr','maxExtensionAtr','atrStopMult','maxHoldTimeMinutes','requireRth','allowShorts'].includes(r.property)),
    callers:scan.methodCalls.filter(r=>['evaluate','saveSettings','createExitContract','checkInvalidationConditions'].includes(r.method)),
    blast:await Promise.all(['modules/EMATrendRetest.js','core/StrategyOrchestrator.js','foundation/ConfigLoader.js'].map(p=>getBlastRadius(p)))};
