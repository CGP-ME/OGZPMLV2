...
(async()=>{
  ...
  const inherited=cp.execFileSync('git',['diff','--binary','--','modules/PropSafeEMAPullback.js','core/StrategyOrchestrator.js','foundation/ConfigLoader.js'],{cwd:root,maxBuffer:8e6});
  ...
  const record={at:new Date().toISOString(),boundary:'Existing Mercury dependency/Serena AST tools; static candidates, not execution completeness',
    ...
    references:scan.propertyRefs.filter(r=>['fastEmaPeriod','pullbackEmaPeriod','trendEmaPeriod','atrPeriod','crossLookbackBars','pullbackLookbackBars','pullbackMinAtr','pullbackMaxAtr','atrStopMult','requireRth','allowShorts','maxHoldTimeMinutes'].includes(r.property)),
    callers:scan.methodCalls.filter(r=>['evaluate','saveSettings','createExitContract','checkInvalidationConditions'].includes(r.method)),
    blast:await Promise.all(['modules/PropSafeEMAPullback.js','core/StrategyOrchestrator.js','foundation/ConfigLoader.js'].map(p=>getBlastRadius(p)))};
