...
# FABLE SECTIONED REVIEW TASK

Read-only. Do not edit files. Do not write files. Do not run PM2. Use available repo-reading/search tools if the CLI permits; otherwise grade only the supplied artifacts and label evidence gaps.

The first max-effort Fable run took too long with no output. This is the fallback that preserves the same mission in a sectioned contract. Do not summarize. Produce a complete evolved Mercury+Fable architecture report. Do not stop at the first plausible critique.
