...
## A17. Loss warnings and loss gates (Sept 6)

> loader refuses to run live unless the guards are on? wtf rip it iout that shouldnt be ther

> what the fuck who is cooldowning my bot

> none and take that shit out for the same reason

> it warninng messages fly at 3 trades lost in a row or 5% then 10% then 15%

> aor 4 or 5 losses

> nothing shuts anything down or stops the bot ewxcept dd during eval

> you can put a timeout user needs to restart it at 50%lost in a day

> one safeguard

> stops from entering flatten losers winners run

> actrually no just halt new entries if we sell something that swing a customer will be jacked

## A18. PID / ML layer (Sept 7)

> keep and wire the PID that + the pattern learning is gunna be the ML layer

## A19. Mercury's method (Sept 7)

> mercury should do this assess the quiestion decide any of its tooling is appropriate for it keep reading until it has read all the comments filing away potential answers while it reads then when exhausted go back over everything and pick the correct one citing everything

> mercury reads everything yes I want it to fully understand before it answers

## A20. Track-record publisher (Sept 7)

> yeah if they are there i wanted them at some point for something maybe piping through to the website idk

## A21. Config architecture (Sept 8)

> were concatenating everything into two configs one a config that is going to be linked to a UI it will be a dynmaic config all the paramenere that customers will be setting and anything that changes in the bot frequesntly and then there will be a statidc config loader

> and were checfkiing the tape on evertyhing while werte doing itthis is my final :wakj of the bot before we start it again forreal this timeP

> do it   [revisioned settings: every save numbered + hashed, every receipt records the revision, positions freeze exit parameters at entry]

## A22. Fees (Sept 8)

> theres only one fee structure because alpaca doesnt charge fees to trade

## A23. Settings activation and per-trade stop edits (Sept 9)

> yes keep slip okay okay on crash okay on crash fix put them in? after restart? can you make them hot swappable like so that a customer can adjust their stop while the trade is running

> yeah the moving the stop would be for that specific trade in time nothing else and not the overall value

## A25. Switching and paper-on-boot (Sept 9)

> build them now so we catch the problems and have the final shape before more problems will arise and there will be things we havent thought of till the end i thought this was alraedy done i think it should boot paper forever and always and not give a fuck about whatever

## A26. Standing rule, singleton, fault halts (Sept 9)

> anything that is a question about defferring till later in the build im gunna say do it now we need the full shape moving forward so we assume all possibilites 
> you saying the singleton? absolutely needs to stay on and prevent a second copy that would be catastrophic 
> recovery with receipt and scream

## A24. Track-record publisher (Sept 9)

> exactly what you put  the usernames acc then yea alpaca or kraken and then live or paper or backtest active or standby i guess 0 min trades date that it is profit target thats alwasys subject to change drawdown is too and then whatever the balance is cant they read from the broker you know what just turn this off
...
3. Nothing shuts the bot down. Nothing flattens on a loss. Warnings only. Two stops in the whole bot: the drawdown limit during an eval, and a daily-loss timeout at 50% lost in a day (settings number, default 50) after which the user must restart it (Sept 6, verbatim). At the timeout: halt new entries only, nothing sold, open positions run their own exits, user restarts (Sept 6, corrected from an earlier flatten-losers line in the same session).
...
- **Settings activation (Sept 9, A23):** hot — a saved setting applies to new decisions immediately, no restart; globals never move an open position's stop (frozen at entry); a customer may edit one specific open trade's stop through the UI — that trade only, that moment only, not the global value — with its own receipt. Slippage retained (not commission). Crash-handler fix and its boot/runtime split: okay. R7 sizing values: put in (Sept 9).
- **PID controller (Sept 7):** keep and wire — PID + pattern learning = the ML layer. Stop 4 census; wiring is its own mission after the walk; its config rows → settings.
...
- Loud warnings at 3, 4, and 5 losses in a row, and at 5%, 10%, 15% down (Sept 6). No stop, no flatten. Per-symbol loss cooldown: deleted everywhere, eval included (Sept 6). Live-requires-eval-guards check in the loader: deleted (Sept 6).
