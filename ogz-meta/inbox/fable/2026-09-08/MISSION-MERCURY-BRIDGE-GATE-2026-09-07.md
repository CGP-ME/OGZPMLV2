...
## Part C — the panel discards correct answers (added Sept 7 after Codex's replays; Fable's earlier "out of scope" line on the panel was the reason the proof could not pass — withdrawn)

What the code does (read at e54a8b8):
- `reviewer-panel.js:14-20` `structuredPanelVerdict`: a plain audit answer with no `verdict` field → `cannot_verify`. Mercury's audit answers never carry that field, so Mercury's seat is `cannot_verify` on every audit regardless of content.
- `ask.js:1018` stores Fable's seat with its pre-recheck parsed verdict (`found_break` on line 31); the recheck (`:1011-1017`) is attached as `fableReview.rechecks` but the seat's `verdict` is never recomputed from it.
- `reviewer-panel.js:161-171` `evaluatePanelAuthority` compares `seat.verdict` across seats → `[cannot_verify, found_break, pass]` → `unverified`.
- `ask.js:1155-1157` attaches Kimi's adjudication (`"convergence"`, `pass`) after `panelRun.authority` is already computed; authority is not recomputed.
Ledger `2026-09-07.jsonl:31` is the receipt: candidate set captured, recheck 50 tool calls / 9 files, all 15 keys NO LIVE READER, Kimi pass + convergence, panel UNVERIFIED.

The change:
10. `structuredPanelVerdict`: an answer without a `verdict` field is `no_claim`, not `cannot_verify`; `no_claim` does not participate in the disagreement comparison (an audit answer is a finding, not a break/pass verdict).
11. After a recheck completes, the Mercury and Fable seats' `verdict`, `evidenceBasis`, and `evidenceChecksPassed` are recomputed from the recheck output before `evaluatePanelAuthority` runs. Pre-recheck values stay in the ledger as history, not as the seat.
12. After Kimi's adjudication is attached, authority is recomputed; a Kimi `pass` with explicit `convergence` and no unresolved disagreement yields FULL when the evidence checks pass.

## Proof — ONE run, then stop (Trey, Sept 7: no more provider spend on repeated replays)
Replay ledger line 20's prompt exactly once. Hard stop on the first UNVERIFIED: no rerun, the ledger line comes back to Fable with the seats and absences printed. Expected on pass: panel FULL, `candidate_set` present, final citations ⊆ candidate set. Then the evidence-free control once (expected UNVERIFIED via `no_mechanical_evidence`). Two runs total.

## Out of scope
Quarantines, identity posture, run-ledger schema, quarantines, identity posture, run-ledger schema, the `--agentic` default, `maxTokens` config coupling (`mercury.config.json` vs `--max-tokens`; noted Sept 7 as friction, separate item), the claude-bridge "break my fix" gate on Claude Code's seat.
...
---
**WHAT I DID:** read `reviewer-panel.js:1-30, 150-175`, `ask.js:1005-1020, 1155-1170` (Part C, Sept 7); read `react-loop.js:555-745` (setup, loop, termination), `config.js:307-311`, both system prompts in `mercury.config.json`; read `doctrine-review.js:1-238`, `ask.js:938-1084` (three call sites and the three seat gates), `reviewer-panel.js:70-95`, `run-ledger.js` receipt fields, `adversarial-review.js:854`; wrote the change from those lines.
**WHAT I DID NOT DO:** land or verify Codex's Part A/B edits (they sit unpushed on the stray branch and are unproven); read the recheck prompt builder in `ask.js` (item 6 tells the executor to cite it), the bridge tests, `llm-client.js:392-396` authority assignment, or the claude-bridge gate.
