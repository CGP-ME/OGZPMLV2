...
**Status:** HOLD — do not execute until Trey names the executor and says go.
...
11. **Boot cosmetics** (Trey Sept 6): the `v14` lock name, the emoji lock lines, checkpoint counters, and the "Desktop Claude + Browser Claude = MERGED" banner come out of the boot path. No behavior.
...
**Open question for Trey (blocks ignition only if the publisher stays on):** the box `.env` lacks all nine track-record/proof-account values; with current code the publisher fails on every trade at `claudito-logger.js:374`, is caught at `:717-726`, and trading continues. Trey rules: what the publisher publishes and to whom; on with values, or delete.
