...
- The VPS checkout is the canonical production-visible repo for agent doctrine and Sourcegraph/DeepSearch reads.
- Windows workspace root may be `C:\Users\og_za\Downloads\OGZPMLV2-rebuild-clean-from-baseline`; Windows, Dropbox, or downloaded baseline copies are secondary evidence unless Trey explicitly names one as the target.
...
- Proof contracts must enumerate every owner of the mechanism being proved, not
  only the files touched by the last patch or the story around the invariant.
  Source: `ogz-meta/inbox/fable/2026-07-20/codex1-summary-pipeline-proof-language-failure.md:87-99`.
...
- Repo-local `.claude/hookify*.local.md` files are current feedback-derived guardrails. Read them before tasks involving approvals, edits, git, Mercury, backtests, fake data, no guessing, session docs, or communication rules.
- Hookify plugin rules belong in the current project `.claude/` directory. Do not treat plugin examples, global hooks, or another checkout's hookify files as repo policy unless current repo-local rules or Trey's current instruction say so.
...
- Do not claim config ownership is consolidated until `npm run scan:config-boundary`
  is clean or remaining exceptions are explicitly ratified and documented. The
  detector is an audit surface, not a runtime guard. Source:
  `ogz-meta/inbox/fable/2026-07-20/codex1-summary-p1-config-boundary-detector.md:108-122`;
  `ogz-meta/inbox/fable/2026-07-20/codex1-summary-config-boundary-detector-mercury-followup.md:104-108`.
...
- For `AGENTS.md` and canonical doctrine refreshes, source from the live VPS tree first: root `AGENTS.md`, `ogz-meta/AGENTS.md`, `.claude/hookify*.local.md`, `ogz-meta/Alignment/`, `ogz-meta/BACKTEST-OPS.md`, latest session docs, and current source/help.
- Ledger, proposal, alignment intake, and old session material are candidate evidence until verified against current source or explicitly promoted by Trey. If unsure, queue the candidate under `ogz-meta/inbox/codex/<YYYY-MM-DD>/` with source paths and a TODO/ruling note instead of promoting it into canonical doctrine.
