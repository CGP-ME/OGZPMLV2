...
- This VPS checkout is the canonical production-visible repo. Local Windows, Dropbox, or downloaded baseline copies are secondary evidence unless Trey explicitly names them as the target.
- Sourcegraph/DeepSearch reads the VPS filesystem at `/opt/ogzprime/OGZPMLV2`; pushing to GitHub is audit trail, not proof a local file exists here.
...
- For canonical doc updates, source rules from this VPS tree first: `AGENTS.md`, `ogz-meta/AGENTS.md`, `ogz-meta/Alignment/`, `.claude/hookify*.local.md`, current source/help, `ogz-meta/BACKTEST-OPS.md`, and latest session docs. Do not promote local baseline/Dropbox docs or ledger/proposal intake without verifying against this tree.
