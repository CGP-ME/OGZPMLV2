# OGZPMLV2 Sourcegraph Deep Search Archive — 2026-05-20

**Status:** ARCHIVED — DO NOT MODIFY UNTIL POST-APEX EVALUATION
**Source:** Sourcegraph deep search session 309cd7e9-70e7-46a3-88d1-32f1ef6df034
**Branch audited:** `rebuild/clean-from-baseline`
**Quarantine snapshot:** temporary branch `codex/sourcegraph-quarantine-expansion`
...
This document is the verbatim output of three combined Sourcegraph deep-search asks:

1. **Bug investigation pass** — Symbol mislabel + Shorts in long_only mode root cause
2. **Full TRAI/NLP integration audit** — Phases 0 through K mapping the entire post-Apex TRAI build
3. **Silent Killer sweep** — The bonus ask covering bugs that compile clean, log nothing, only surface under live capital

Per operator discipline: this archive is to be referenced post-Apex only. Pre-eval work uses the Priority Stack at the bottom of this doc; do not bury yourself in the integration phases before the eval is done.

The temporary quarantine branch that gave Sourcegraph visibility into legacy files lives at commit SHA (to be filled in) — operator saves this SHA outside the quarantine directory before deleting the temp branch.

---

## Table of Contents

### Section 1 — Bug Investigation Findings (Phases A-D from second prompt)
- [Bug 1: Symbol Mislabeling](#bug-1-symbol-mislabeling)
- [Bug 2: Shorts in long_only mode](#bug-2-shorts-in-long_only-mode)
- [Phase A Findings — Symbol Assignment](#phase-a-findings--symbol-assignment)
- [Phase B Findings — Short Origination](#phase-b-findings--short-origination)
- [Phase C Findings — Report Aggregation](#phase-c-findings--report-aggregation)
- [Phase D Findings — Full Trade Recording Chain](#phase-d-findings--full-trade-recording-chain)

### Section 2 — Full TRAI Integration Audit (Phases 0-K)
- [Phase 0: Repo Manifest](#phase-0-repo-manifest)
- [Phase A: TRAI #1 News + NLP Sentiment](#phase-a-trai-1-news--nlp-sentiment)
- [Phase B: TRAI #2 Whale Watcher](#phase-b-trai-2-whale-watcher)
- [Phase C: TRAI #3 Pattern Modulator Production Verification](#phase-c-trai-3-pattern-modulator-production-verification)
- [Phase D: TRAI #4 Trade Analyst / Performance Analyzer](#phase-d-trai-4-trade-analyst--performance-analyzer)
- [Phase E: TRAI #5-9 Mover Stack + Hitch](#phase-e-trai-5-9-mover-stack--hitch)
- [Phase F: Position Sizer Gap Analysis](#phase-f-position-sizer-gap-analysis)
- [Phase G: Regime Detector Wiring](#phase-g-regime-detector-wiring)
- [Phase H: Confidence Pipeline Architecture](#phase-h-confidence-pipeline-architecture)
- [Phase I: Pattern Bank Integration](#phase-i-pattern-bank-integration)
- [Phase J: Contamination & Architectural Sweep](#phase-j-contamination--architectural-sweep)
- [Phase K: Master Integration Sequence](#phase-k-master-integration-sequence)

### Section 3 — Silent Killer Sweep (Bonus Ask)
- [KILL 1: Confidence Math Bug](#kill-1-confidence-math-bug-high)
- [KILL 2: exit_partial Trace](#kill-2-exit_partial-trace-medium)
- [KILL 3: FibonacciDetector Orphaned](#kill-3-fibonaccidetector-orphaned-low)
- [KILL 4: RiskManager Bypass](#kill-4-riskmanager-bypass-medium)
- [KILL 5: SELL with No Matching BUY](#kill-5-sell-with-no-matching-buy-high)
- [KILL 6: adjustedConfidence 100x Wrong](#kill-6-adjustedconfidence-100x-wrong-low)
- [KILL 7: nearestStructure Null Reaches MPM](#kill-7-neareststructure-null-reaches-mpm-medium)
- [What Is Actually Clean (6 items)](#what-is-actually-clean)

### Section 4 — Pre-Eval Priority Stack
- [Priority Stack for Pre-Eval](#priority-stack-for-pre-eval)

---

## Critical Headline Findings (Pre-Eval)

Before diving into the archive, the items the operator needs to action **BEFORE running the next P0 backtest sweep** to ensure the anchor is meaningful:

1. **Confidence clamp at TradingLoop.js:135** — confidence can exceed 1.0 today. 1-line fix. P&L math unreliable until landed.
2. **KILL 5 emergency-reset risk** — SELL with no matching BUY wipes balance state. HIGH risk during eval if symbol mislabel or state desync triggers.
3. **adjustedConfidence /100 at TradingLoop.js:419** — losing-strategy confidence in ledger is 100x inflated. 1-character fix.
4. **nearestStructure null reaches MPM** — trailing stops are structure-blind. 1-line fix.
5. **Confirm RISK_MANAGER_BYPASS=false in .env** — grep check before every eval run.
6. **5 dead feature flags** — ENABLE_SHORTS, ENABLE_DYNAMIC_SIZING, ENABLE_ARBITRAGE, ENABLE_HEDGING, ENABLE_REGIME declared but never consumed.
7. **Symbol mislabel still possible** — Phase A findings document the root cause path.

Post-Apex TRAI integration work is in Section 2. Don't touch until eval is done.

---

# Section 1 — Bug Investigation Findings



## Raw Sourcegraph Output (Verbatim)

Below is the complete unedited Sourcegraph deep search session output. Operator prompts are interspersed with Sourcegraph findings.
