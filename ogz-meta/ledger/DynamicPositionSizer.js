...
class DynamicPositionSizer {
  constructor(config = {}) {
    ...
    this.patternMultipliers = config.patternMultipliers ?? {
      ...
      quarantined: 0.00,   // Proven loser — BLOCK THE TRADE
  ...
  calculate(params) {
    ...
    // ── BLOCK CHECK: quarantined pattern = no trade ──
    if (patternMultiplier === 0) {
      ...
      return {
        ...
        capped: false,
        blocked: true,
        reason: `Blocked — pattern quarantined (status: ${patternStatus}, WR: ${patternWinRate !== null ? (patternWinRate * 100).toFixed(1) + '%' : 'N/A'})`,
      };
    }
  ...
  getStats() {
    return {
      ...
      cappedRate: this.calculations > 0
        ...
        : '0%',
      blockedRate: this.calculations > 0
        ? (this.blockedCount / this.calculations * 100).toFixed(1) + '%'
        : '0%',
      config: {
  ...
  printStats() {
    ...
    console.log(`  Blocked (quarantine): ${s.blockedCount} (${s.blockedRate})`);
