...
async function executeMission(missionFile, options = {}) {
  ...
  if (isDryRun) {
    ...
    const branchName = `mission-${missionId}-fixes`;
    console.log(`\n🔒 SAFETY: Creating branch ${branchName}`);
    ...
    try {
      ...
      if (currentBranch === 'master' || currentBranch === 'main') {
        console.log('   Currently on master/main - creating mission branch...');
        ...
        console.log('   🔒 Master branch protected - all changes will be on mission branch');
      } else {
        console.log(`   ⚠️  Already on branch: ${currentBranch}`);
        console.log('   Continuing on current branch...');
      }
    } catch (error) {
      console.error('\n❌ FATAL: Could not create safety branch');
      console.error('   This executor MUST NOT run on master!');
  ...
  try {
    ...
    if (currentBranch.startsWith('mission-')) {
      ...
      console.log('   Master/main is protected!');

      console.log('\n📋 Next steps:');
      console.log('1. Review the diff above carefully');
      console.log('2. Commit and push to create a PR:');
      console.log('   git add .');
      console.log(`   git commit -m "Mission ${missionId}: <description>"`);
      console.log(`   git push -u origin ${currentBranch}`);
      console.log('3. Create pull request on GitHub:');
      console.log(`   gh pr create --title "Mission ${missionId}: Pattern memory fixes" --body "Auto-generated by support mission"`);
      console.log('4. Or if you want to abandon:');
      console.log('   git checkout master');
      console.log(`   git branch -D ${currentBranch}`);
      console.log('5. To clean up artifacts:');
      console.log(`   node ogz-meta/janitor.js --manifest ${manifestPath}`);

      console.log('\n🚨 NEVER merge directly to master without review!');
    } else {
