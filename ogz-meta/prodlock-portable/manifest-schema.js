 ...
 * Note: Fields prefixed with _ (like _sessionForm) are runtime-only
 * and are stripped before writing to disk.
 */
function saveManifest(manifest, path) {
  const fs = require('fs');
  // Strip runtime-only fields (underscore prefix) before persisting
  const toSave = {};
  for (const key of Object.keys(manifest)) {
    if (!key.startsWith('_')) {
      toSave[key] = manifest[key];
    }
  }
  fs.writeFileSync(path, JSON.stringify(toSave, null, 2));
}
