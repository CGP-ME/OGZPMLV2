...
- **EXIT_SYSTEM Feature Flag - Single Exit System Selector (EXIT_CLEANUP_001)** - run-empire-v2.js, config/features.json (ARCHITECTURE FIX) - 2026-02-04
  - **Problem:** 8 competing exit systems fighting each other caused penny P&L on $500 positions
  - **Exit systems identified:**
    1. TradeIntelligenceEngine (13-dimension analysis)
    2. PatternExitModel (pattern-based exits)
    3. MaxProfitManager (tiered exits + trailing stops)
    4. Hard Stop at -1.5% (ALWAYS ON - safety net)
    5. Stale Trade "shit or get off the pot" (30min dead zone exit)
    6. Fee Buffer exit (profitable above fees)
    7. TradingBrain sell signal exits
    8. Gradual loss exit (5min hold + losing)
  - **Fix:** Added `EXIT_SYSTEM` feature flag to `config/features.json`
  - **Options:** `maxprofit` (default), `intelligence`, `pattern`, `brain`, `legacy`
  - **File:** `config/features.json` lines 154-170
  - **File:** `run-empire-v2.js` line 419 (initialization), lines 2001-2170 (all exit conditionals)
  - **Before:** All 8 exit systems active simultaneously, first one to fire wins
  - **After:**
    ```javascript
  ...
  - Hard stop at -1.5% NOT wrapped - always active as safety net
  - **Backtest result:** maxprofit-only = -$0.52 vs legacy = -$500.62 (1000x improvement)
  - **Usage:** Set `EXIT_SYSTEM=intelligence` env var or change `config/features.json` activeSystem
  - **Commit:** `fe894be`
