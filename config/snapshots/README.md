...
Per-run snapshots of the resolved TradingConfig — captured at trade-decision time so every decision ledger entry has an auditable config fingerprint.
...
When Phase 6 of the config consolidation migration lands (`ogz-meta/ledger/CONFIG-CONSOLIDATION-SPEC.md §4.8`), every backtest run and every live trade will emit a snapshot JSON here — the fully resolved TradingConfig after `.env` + CLI `--set` + `--profile` + `--override` layers are merged and frozen. Each snapshot is keyed by the config fingerprint (content hash), so identical configurations dedupe automatically.
